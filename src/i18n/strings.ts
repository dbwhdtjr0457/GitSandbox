...
const ko: LocaleStrings = {
  ...
  tutorial: {
    ...
    quickTips: [
      '왼쪽 그래프와 오른쪽 에디터/터미널을 함께 보면 상태를 한눈에 이해할 수 있습니다.',
      '처음에는 help를 실행하고, 이후 명령어를 단계별로 실행해 보세요.',
      '브랜치 이동, reset, merge 결과가 그래프와 에디터에 바로 반영됩니다.',
    ],
    ...
    workflowLines: [
      'help: 지원 명령어 목록 확인',
      'checkout/detached: 다른 커밋으로 HEAD 이동 후 계속 편집 가능',
      'merge: FF와 non-FF 병합 동작 확인',
      'conflict: 충돌/결과 흐름 및 에디터 상태 확인',
    ],
    closeHint: '입력한 명령은 터미널 히스토리에 그대로 기록되며 한 줄씩 결과가 출력됩니다.',
    demoHint: '데모 시나리오 버튼을 통해 실습을 한 번에 진행할 수 있습니다.',
  },
  demo: {
    title: '데모 시나리오',
...
const en: LocaleStrings = {
  ...
  tutorial: {
    ...
    quickTips: [
      'Use the left graph and right editor/terminal together to understand repository state and outputs.',
      'Run help first, then execute commands step by step.',
      'Branch switch, reset, and merge updates are reflected immediately in both graph and editor.',
    ],
    ...
    workflowLines: [
      'help: Check supported command list',
      'checkout/detached: Move HEAD to another commit and continue typing',
      'merge: Observe FF and non-FF behavior',
      'conflict: Check conflict flow and editor result',
    ],
    closeHint: 'Inputs are appended to terminal history and rendered line by line.',
    demoHint: 'Use demo scenarios to run full flows with one click.',
  },
  demo: {
